...
2. В SQL Editor выполните по порядку содержимое файлов [`supabase/migrations/202608200001_audit_board.sql`](supabase/migrations/202608200001_audit_board.sql), [`supabase/migrations/202608200002_add_roadmap_tracker.sql`](supabase/migrations/202608200002_add_roadmap_tracker.sql), затем [`supabase/seed.sql`](supabase/seed.sql). Для уже существующей базы достаточно применить вторую миграцию: она безопасно добавит 56 roadmap-задач к каждой существующей доске. Альтернатива с локальным `psql`:
...
Workflow выполняет `npm ci`, тесты и production-сборку, а в отдельной временной локальной базе Supabase — все 72 pgTAP-проверки. Деплой начинается только после обоих успешных заданий, после чего загружает только `dist/` штатными GitHub Pages actions. В артефакт не входят `node_modules`, тесты, `.git`, SQL-файлы Supabase, локальная конфигурация или ключи с расширенными правами.
...
`npm run test:db` запускает `supabase test db` и выполняет файл `supabase/tests/database.test.sql` с 72 pgTAP-проверками на свежей локальной базе. Workflow фиксирует CLI версии `2.84.2` через официальный `supabase/setup-cli@v2`.
